...
sensor_msgs::ImagePtr makeRGBA8Image(const std_msgs::Header& header, std::size_t width, std::size_t height)
{
  ...
  , header_seq_(0)
{
  ...
  capture_settings_controller_ =
      std::make_unique<Capture3DConfigController>(nh_, camera_, "settings", max_capture_acquisitions);
  ...
  capture_2d_config_controller_ = std::make_unique<Capture2DConfigController>(nh_, camera_, "settings_2d", 1);
  ...
  snr_image_publisher_ = image_transport_.advertise("snr/image", 1, use_latched_publisher_for_snr_image_);
...
bool ZividCamera::captureServiceHandler(Capture::Request&, Capture::Response&)
{
  ...
  ROS_INFO("Capturing with %zd acquisitions", settings.acquisitions().size());
...
bool ZividCamera::capture2DServiceHandler(Capture2D::Request&, Capture2D::Response&)
{
  ...
  const auto settings2D = capture_2d_config_controller_->zividSettings();
...
void ZividCamera::publishFrame(Zivid::Frame&& frame)
{
  ...
  if (publish_points_xyz || publish_points_xyzrgba || publish_color_img || publish_depth_img || publish_snr_img)
  {
    ...
    if (publish_snr_img)
    {
      ...
      snr_image_publisher_.publish(makeSnrImage(header, point_cloud));
    }
  }
}
...
void ZividCamera::publishPointCloudXYZRGBA(const std_msgs::Header& header, const Zivid::PointCloud& point_cloud)
{
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - t1).count();
  ROS_INFO("%s took %lu us", __func__, duration);
}

sensor_msgs::ImageConstPtr ZividCamera::makeColorImage(const std_msgs::Header& header,
                                                       const Zivid::PointCloud& point_cloud)
{
  auto msg = makeRGBA8Image(header, point_cloud.width(), point_cloud.height());
  msg->data.resize(msg->step * msg->height);
  point_cloud.copyData<Zivid::ColorRGBA>(reinterpret_cast<Zivid::ColorRGBA*>(msg->data.data()));
  return msg;
}
...
{
  auto msg = makeRGBA8Image(header, image.width(), image.height());
                                                       ...
                                                       const Zivid::PointCloud& point_cloud)
{
  ...
  msg->data.resize(msg->step * msg->height);
  point_cloud.copyData<Zivid::PointZ>(reinterpret_cast<Zivid::PointZ*>(msg->data.data()));
  return msg;
}

sensor_msgs::ImageConstPtr ZividCamera::makeSnrImage(const std_msgs::Header& header,
                                                     const Zivid::PointCloud& point_cloud)
{
  auto msg = boost::make_shared<sensor_msgs::Image>();
  fillCommonMsgFields(*msg, header, point_cloud.width(), point_cloud.height());
  ...
  msg->step = static_cast<uint32_t>(4 * point_cloud.width());
  msg->data.resize(msg->step * msg->height);
  point_cloud.copyData<Zivid::SNR>(reinterpret_cast<Zivid::SNR*>(msg->data.data()));
  return msg;
}
